...
let load host port user password scheme =
    ...
    |> List.map
                        (fun table ->
                            sprintf "Getting foreign keys from %s." table |> stderr.WriteLine
                            let query= sprintf """SELECT KCU1.COLUMN_NAME,
                             ...
                             WHERE RC.TABLE_NAME = '%s'""" table
                            use comm = conn.CreateCommand()
                            comm.CommandText<- query
                            use rdr = comm.ExecuteReader()
                            (table, [
                                while rdr.Read() do
                                    yield (rdr.GetString(0), Foreign.Yes(rdr.GetString(1),rdr.GetString(2)))
                            ] |> Map.ofList)
                   ) |> List.map
            (fun (table, relations) ->
                ...
                {Name=table; Columns=
                    [
                        ...
                        (fun (name,dt,primary,nullable) ->
                            let foreign name =
                                match relations |> Map.tryFind name with
                                |Some f -> f
                                |None -> Foreign.No
                            match primary with
                            |false ->   {Table=table;Name=name;Type= dt;Primary=Primary.No;Foreign = foreign name; Nullable = nullable}
                            |true ->
                                ...
                                use rdr = comm.ExecuteReader()
                                if rdr.Read() then
                                    {Table=table;Name=name;Type=dt;Primary=Primary.Yes (Generators.sequentialGenerator (rdr.GetInt32(0)));Foreign = foreign name; Nullable=nullable}
                                else
                                    {Table=table;Name=name;Type= dt;Primary=Primary.No;Foreign = foreign name; Nullable = nullable}
                   )
                       }
                )