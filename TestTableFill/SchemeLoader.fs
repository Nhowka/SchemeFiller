    ...
    |> List.map (fun (table, relations) -> 
           ...
           { Name = table
             Columns = 
                 [ use comm = conn.CreateCommand()
                   ...
                   while rdr.Read() do
                       ...
                       let dt = 
                           ...
                           | "blob" | "char" | "enum" | "longtext" | "longblob" | "mediumtext" | "mediumblob" | "set" | "text" | "varchar" -> 
                               Text(rdr.GetInt32(2))
                 ...
                 |> List.map (fun (name, dt, primary, nullable) -> 
                        let foreign name = 
                        ...
                        | true -> 
                            ...
                            if rdr.Read() then 
                                { Table = table
                                  Name = name
                                  Type = dt
                                  Primary = Primary.Yes(Generators.sequentialGenerator (rdr.GetInt32(0)))
                                  Foreign = foreign name
                                  Nullable = nullable }
                            else 
                                { Table = table
